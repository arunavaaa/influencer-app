...
export default function LoginPage() {
  ...
  async function signInWithEmail() {
    ...
    if (data?.role === 'brand') router.push('/brand/dashboard')
    else if (data?.role === 'creator') router.push('/dashboard')
      ...
      <div className="w-full lg:w-[480px] flex flex-col justify-center bg-white px-8 py-12 lg:px-14 relative">
        ...
        <div className="w-full max-w-[360px] mx-auto">
          ...
          <div className="mb-4">
            ...
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" onKeyDown={e => e.key === 'Enter' && signInWithEmail()}
          ...
          <div className="mb-6">
            ...
            <div className="relative">
              <input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="Your password" onKeyDown={e => e.key === 'Enter' && signInWithEmail()}
          ...
          <div className="text-center space-y-2">
            <p className="text-[14px] text-[#6A6C6A]">New creator? <Link href="/signup?role=creator" className="text-[#163300] font-bold hover:underline">Create your profile free →</Link></p>
            <p className="text-[14px] text-[#6A6C6A]">Brand? <Link href="/signup?role=brand" className="text-[#163300] font-bold hover:underline">Join as a brand →</Link></p>
          </div>
        </div>
      </div>
    </div>
